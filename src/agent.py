...
class ZerePyAgent:
    ...
    def prompt_llm(self, fine_tuned_prompt: str, prompt: str, system_prompt: str = None, rag: bool = None, user_risk: str = None) -> str:
        """Generate text using the configured LLM provider"""
        system_prompt = system_prompt or self._construct_system_prompt()
        final_prompt = f"{prompt} {fine_tuned_prompt}"
        
        if rag:
            self.connection_manager.perform_action(
                connection_name=self.model_provider,
                action_name="load-rag-data",
                params=["https://kolective-backend.vercel.app/api/kol", user_risk]
            )
            
            print("Successfully loaded RAG DATA")
        
            return self.connection_manager.perform_action(
                connection_name=self.model_provider,
                action_name="generate-text",
                params=[final_prompt, system_prompt, "gpt-4o-mini-2024-07-18", True]
            )
            ...
            return self.connection_manager.perform_action(
                connection_name=self.model_provider,
                action_name="generate-text",
                params=[final_prompt, system_prompt]
            )
